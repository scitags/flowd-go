...
// Plundered from https://github.com/tailscale/tailscale/blob/cf40cf5ccb1be1ab931b427f7f83bba3214ace79/net/stun/stun.go
package stun
...
func GetPubIPOverSTUN(c Config, family int, localAddr net.IP) (net.IP, error) {
	...
	case unix.AF_INET:
		network = "udp4"
		lAddr, err = net.ResolveUDPAddr(network, localAddr.String()+":0")
	case unix.AF_INET6:
		network = "udp6"
	...
	for _, srvAddr := range c.StunServers {
		rAddr, err := net.ResolveUDPAddr(network, srvAddr)
		if err != nil {
			slog.Warn("error resolving remote address", "err", err)
			continue
		}

		conn, err := net.DialUDP(network, lAddr, rAddr)
		if err != nil {
			slog.Warn("error dialing", "err", err)
			continue
		}

		txId := NewTxID()
		_, err = conn.Write(Request(txId))
		if err != nil {
			slog.Warn("error sending STUN request", "err", err)
			continue
		}

		if err := conn.SetReadDeadline(time.Now().Add(5 * time.Second)); err != nil {
			slog.Warn("error setting read deadline", "err", err)
			continue
		}
		...
		if err != nil {
			slog.Warn("error reading", "err", err)
			continue
		}

		txIdResp, addr, err := ParseResponse(buff[:n])
		if err != nil {
			slog.Warn("error parsing STUN response", "err", err)
			continue
		}
